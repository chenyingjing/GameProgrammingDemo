...
while(TRUE)
	...
	if (PeekMessage(&msg,NULL,0,0,PM_REMOVE))
       ...
       sprintf_s(buffer,"Up Arrow: = %d   ",KEYDOWN(VK_UP));
       TextOut(hdc, 0,0, buffer, strlen(buffer));

       sprintf_s(buffer,"Down Arrow: = %d   ",KEYDOWN(VK_DOWN));
       TextOut(hdc, 0,16, buffer, strlen(buffer));

       sprintf_s(buffer,"Right Arrow: = %d   ",KEYDOWN(VK_RIGHT));
       TextOut(hdc, 0,32, buffer, strlen(buffer));

       sprintf_s(buffer,"Left Arrow: = %d   ",KEYDOWN(VK_LEFT));
