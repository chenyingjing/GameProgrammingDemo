...
printf("dd_pixel_format:%d", dd_pixel_format);
...
if ((!fopen_s(&fp_error, filename,"w")))
   return(0);
}
...
ctime_s(timeline, 100, &(timebuffer.time));
...
if (!(fopen_s(&fp_error, filename,"a+")))
   return(0);
}
...
SetFilePointer((HANDLE)file_handle, -(int)(bitmap->bitmapinfoheader.biSizeImage), 0, FILE_END);
