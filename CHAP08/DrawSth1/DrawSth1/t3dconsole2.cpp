...
int Game_Main(void *parms)
{
	...
	Draw_Pixel(200, 200, 201, (UCHAR *)ddsd.lpSurface, ddsd.lPitch);

	HLine(10, 400, 300, 200, (UCHAR *)ddsd.lpSurface, ddsd.lPitch);

	VLine(10, 300, 200, 200, (UCHAR *)ddsd.lpSurface, ddsd.lPitch);
	...
	Draw_Rectangle(200, 250, 300, 350, 200, lpddsback);

	// flip the surfaces
	DDraw_Flip();

	Draw_Text_GDI("This is a test", 200, 300, RGB(255, 255, 255), lpddsprimary);
...
void DrawPolygon(UCHAR * lpSurface, LONG lPitch)
{
	...
	Scale_Polygon2D(&fivePoly, scaleXY, scaleXY);
...
float GetScaleXY()
{
	static float scaleXYArray[] = {1, 0.9, 0.8, 0.7, 0.6, 0.5};
	static int scaleXYArrayIndex = 0;
	static int scaleXYArrayStep = 1;
	int scaleXYArrayLength = sizeof(scaleXYArray)/sizeof(scaleXYArray[0]);
	float returnValue = scaleXYArray[scaleXYArrayIndex];
	if (scaleXYArrayIndex >= scaleXYArrayLength)
	{
		scaleXYArrayStep = -1;
	}
	if (scaleXYArrayIndex < 0)
	{
		scaleXYArrayStep = 1;
	}
	scaleXYArrayIndex += scaleXYArrayStep;
	return returnValue;
}
